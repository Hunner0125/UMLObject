...
void CRectangle::Draw( CDC *pdc )
{
 ...
 //       pOldPen=pdc-> SelectObject(&p);     //把画笔选入DC，并保存原来画笔
	...
	int l = (m_End.y - m_Start.y) * 0.25;

	CPoint m1,n1,m2,n2;
	m1.SetPoint(m_Start.x,m_Start.y+l);
	n1.SetPoint(m_End.x,m_Start.y+l);
	pdc->MoveTo(m1);
	pdc->LineTo(n1);

	m2.SetPoint(m_Start.x,m_Start.y+2*l);
	n2.SetPoint(m_End.x,m_Start.y+2*l);
	pdc->MoveTo(m2);
	pdc->LineTo(n2);
...
bool CRectangle::IsOn(ConnectPoint *pt)
{
	ConnectPoint *temp = NULL;
	for(int i = 0; i < 8; i++)
	{
...
void CRectangle::Serialize(CArchive& ar)
{
	if(ar.IsStoring())
	{
		ar<<m_Start<<m_End<<m_Name<<m_AdjustPoint;
	}
	else
	{
		ar>>m_Start>>m_End>>m_Name>>m_AdjustPoint;
	...
	for (is = feature.begin(); is != feature.end(); ++is)
	{
		Feature ft = *is;
		ft.Serialize(ar);
	}*/
	
	m_Relate.Serialize(ar);
...
void CRectangle::Relate(Graph *g)
{
